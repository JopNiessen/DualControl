...
class SoftActorCritic:
    ...
    def __init__(self, observation_size, control_size, key,
        ...
        self.passive_policy = PolicyNetwork(self.n_obs, keys[3])
        self.pending_buffer = list()
        self.use_passive_policy = True
    ...
    def update_passive_policy(self):
        mu_layer = self.PI.model.mu_layer
        log_std_layer = self.PI.model.log_std_layer
        self.passive_policy = eqx.tree_at(lambda policy:policy.mu_layer, self.PI.model, replace=mu_layer)
        self.passive_policy = eqx.tree_at(lambda policy:policy.log_std_layer, self.PI.model, replace=log_std_layer)
    ...
    def get_control(self, state, key):
        ...
        """
        if self.use_passive_policy:
            return self.passive_policy(state, key)
        else:
    ...
    # def update(self, state_transition, key, tracking=True, learning=True, show=False):
    #     """
    #     Update SAC
    ...
    #     if learning:
    #         self.train(key, batch_size=self.batch_size, n_epochs=self.n_epochs, show=show)

    #     if tracking:
    #         self.tracker.add([state[0], state[1], control, reward])
    
    def light_update(self, state_transition, tracking=True):
        state, control, cost, next_state = state_transition
        reward = self.cost_to_normalized_reward(cost)

        self.pending_buffer.append((state, control, reward, next_state))

        if tracking:
            self.tracker.add([state[0], state[1], control, reward])
    
    def hard_update(self, state_transition, key, tracking=True, learning=True, show=False):
        self.light_update(state_transition, tracking)

        for transition in self.pending_buffer:
    ...
    def update(self, state_transition, key, tracking=True, learning=True, show=False):
        if self.training_interval == 10:
            self.training_interval = 0
            self.update_passive_policy()
            self.hard_update(state_transition, key, tracking=tracking, learning=learning, show=show)
        else:
